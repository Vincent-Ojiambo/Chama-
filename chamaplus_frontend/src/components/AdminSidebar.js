...
function AdminSidebar({ sidebarMinimized, setSidebarMinimized, isMobile = false, sidebarOpen = false, onToggle, className = "" }) {
  ...
  const adminMenuItems = [
    ...
    { name: "chamas", label: "Manage Chama", icon: <Briefcase size={20} />, path: "/admin/chamas" },
    ...
    { name: "loans", label: "Loans", icon: <CreditCard size={20} />, path: "/admin/loans" },
    { name: "meetings", label: "Meetings", icon: <Calendar size={20} />, path: "/admin/meetings" },
    { name: "reports", label: "Reports", icon: <PieChart size={20} />, path: "/admin/reports" },
    ...
    >
      ...
      {isMobile && (
        <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-green-400 via-blue-400 to-purple-400 rounded-tr-2xl shadow-md mb-4">
          <div className="flex items-center">
            ...
            <span className="text-xl font-extrabold text-white tracking-wide drop-shadow">Admin Panel</span>
          </div>
          <button
      ...
      {!isMobile && !sidebarMinimized && (
        <div className="px-6 pb-6 mb-2">
          <div className="flex items-center">
            <div className="p-2 bg-gradient-to-r from-green-400 to-blue-500 rounded-lg shadow-md mr-3">
              <Shield size={24} className="text-white" />
            </div>
            <span className="text-lg font-extrabold bg-gradient-to-r from-green-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
              Admin Panel
            </span>
          </div>
        </div>
        ...
        <ul className="space-y-1">
          {adminMenuItems.map((item) => (
            <li key={item.name}>
              ...
              >
                <span className={`text-xl group-hover:scale-110 transition-none ${
                  location.pathname.startsWith(item.path) 
                    ? (isMobile ? "text-white" : "text-green-700") 
                    : (isMobile ? "text-white/70" : "text-gray-400")
                }`}>
                  {React.cloneElement(item.icon, { size: 20 })}
                </span>
                {(!sidebarMinimized || isMobile) && (
                  <span className="truncate ml-3 text-base font-semibold">
                    {item.label}
                  </span>
                )}
              </button>
